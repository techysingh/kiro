...
2. Edit the `.env` file with your configurations:
   ...
   - `OVPN_DATA`: Path to OpenVPN data directory
   - `OVPN_SERVER_URL`: Your VPN server domain
   - `HAPROXY_STATS_USER`: Username for HAProxy statistics
   - `HAPROXY_STATS_PASSWORD`: Password for HAProxy statistics
   - `LETSENCRYPT_EMAIL`: Email for Let's Encrypt notifications
   - `DOMAIN`: Your base domain name
   - `TZ`: Your timezone
